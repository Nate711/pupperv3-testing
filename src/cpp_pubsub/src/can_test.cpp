...
int main()
{
    ...
    auto motor_interface = std::unique_ptr<MotorInterface<K_SERVOS_PER_CHANNEL>>(new MotorInterface<K_SERVOS_PER_CHANNEL>({CANChannel::CAN0, CANChannel::CAN1}));
    ...
    while (!quit.load())
    {
        ...
        auto stop = time_now();
        std::cout << "Angle request (ns): " << duration_ns(stop - start) << "\t"; // takes 80us with large variance
        ...
        std::cout << "Retrival time (ns):\t" << duration_ns(retrieve_end - retrieve_start) << "\t"; // 3000ns average when adding angles
