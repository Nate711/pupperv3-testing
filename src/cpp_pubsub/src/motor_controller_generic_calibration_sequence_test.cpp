...
int main(int argc, char *argv[]) {
  ...
  pupperv3::MotorController<N_ACTUATORS> controller(
      ...
      /*motor_interface=*/std::move(interface), true, true);
  controller.begin();
  std::cout << "Initialized controller." << std::endl;

  int calibration_direction = 1.0;

  auto default_params = pupperv3::MotorController<N_ACTUATORS>::kDefaultCalibrationParams;
  // controller.calibrate_motor(quit, 0, default_params);

  // controller.calibrate_motor(quit, 1, default_params);

  // controller.calibrate_motor(quit, 2, default_params);
  // controller.calibrate_motors(quit, default_params);
  controller.calibrate_motors(quit, {0, 2, 4}, default_params);
  controller.calibrate_motors(quit, {1, 3, 5}, default_params);
