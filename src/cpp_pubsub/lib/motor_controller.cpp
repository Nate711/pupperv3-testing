...
void MOTOR_CONTROLLER::position_control(
    const MOTOR_CONTROLLER::ActuatorCommand &goal_positions)
{
...
TEMPLATE_HEADER
void MOTOR_CONTROLLER::position_control(
    const MOTOR_CONTROLLER::ActuatorCommand &goal_positions, float max_speed)
{
...
void MOTOR_CONTROLLER::velocity_control(
    const MOTOR_CONTROLLER::ActuatorCommand &vel_targets)
{
...
TEMPLATE_HEADER
typename MOTOR_CONTROLLER::RobotMotorData
...
TEMPLATE_HEADER
int MOTOR_CONTROLLER::flat_size()
{
  return kServosPerChannel * num_can_buses_;
}
...
void MOTOR_CONTROLLER::calibrate_motors(const std::atomic<bool> &should_stop)
{
  ...
  // TODO: make this cal directions a parameter so that user isn't fixed to 12 motors or this pattern
  ...
  // ----------------- SUPER DUPER IMPORTANT ------------------------//
  while (!should_stop &&
         !std::any_of(loops_at_endstop.begin(),
                      ...
                      calibrated))
  {
    ...
    for (size_t bus_idx = 0; bus_idx < num_can_buses_; bus_idx++)
    {
      ...
      for (int motor_id = 1; motor_id <= kServosPerChannel; motor_id++)
      {
        ...
        std::cout << "p: " << output_rads << " v*: " << command_velocity << " v: " << output_rad_per_sec << " I: " << current << " ticks: " << loops_at_endstop.at(idx) << " endstop: " << endstop_positions.at(idx);
      }
    }
    std::cout << std::endl;
...
TEMPLATE_HEADER
void MOTOR_CONTROLLER::blocking_move(const std::atomic<bool> &should_stop,
                                     float max_speed,
                                     float move_speed_kp,
                                     ...
                                     int wait_ticks)
{
  ...
  auto float_within_tolerance = [speed_tolerance](float f)
  { return std::abs(f) < speed_tolerance; };

  auto vec_within_tolerance = [float_within_tolerance](std::vector<float> v)
  { return std::all_of(v.begin(), v.end(), float_within_tolerance); };
  ...
  {
    // do a position control with set max speed
    // wait until positions are within tolerance or speed gets low?
    ...
    for (size_t bus_idx = 0; bus_idx < num_can_buses_; bus_idx++)
    {
      ...
      for (int motor_id = 1; motor_id <= kServosPerChannel; motor_id++)
      {
        actuator_vels.at(flat_index(bus_idx, motor_id - 1)) = motor_data.at(bus_idx).at(motor_id - 1).common.output_rads_per_sec;
        std::cout << actuator_vels.at(flat_index(bus_idx, motor_id - 1)) << " ";
      }
    }
    std::cout << std::endl;
    ticks++;
